...
std::string C1::getInfo(){
    return "2 discrete inputs + action";
}

int C1::getNumberOfGenes(){
    return 60;
}

int C1::getActionLayerStart(){
    return 30;
}

std::string C1::getType(){
    return "1";
}

float C1::activation(float x){
    // sigmoid
    return 2.0/(1.0+exp(-2.0*x))-1.0;
}

std::vector<float> C1::compute(std::vector<float> input, std::vector<float> genome){
    ...
    output.push_back(p_output_layer[1] > 0.0 ? 1.0 : 0.0);
