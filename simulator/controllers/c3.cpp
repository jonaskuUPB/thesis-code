...
std::string C3::getInfo(){
    return "4 dsicrete + action";
}

int C3::getNumberOfGenes(){
    return 112;
}

int C3::getActionLayerStart()
{
    return 70;
}

std::string C3::getType(){
    return "3";
}

float C3::activation(float x){
    // sigmoid
    return 2.0/(1.0+exp(-2.0*x))-1.0;
}

std::vector<float> C3::compute(std::vector<float> input, std::vector<float> genome){
    std::vector<float> output;

    // prediction network
    int p_n_input_nodes = 5;
    ...
    int p_n_first_hidden_nodes = 6;
    float p_first_hidden_layer[p_n_first_hidden_nodes];
    int p_first_hidden_layer_start = 10;
    ...
    output.push_back(0.5 * p_output_layer[2] + 0.5);
    output.push_back(0.5 * p_output_layer[3] + 0.5);

    // action network
    int action_weights_start = 70;
    int a_n_input_nodes = 5;
    ...
    int a_first_hidden_layer_start = 80;
